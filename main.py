...
# Transfer weights as edge labels
for node in G.nodes():
    # Specific attribute for node type
    if 'M' in node:
        net.add_node(node, label=str(node), title=f"Node {node}", color="green", shape="circle", label_highlight_color="black")
    if 'N' in node:
        net.add_node(node, label=str(node), title=f"Node {node}", color="red", shape="circle", label_highlight_color="black")
    if 'X' in node:
        net.add_node(node, label=str(node), title=f"Node {node}", color="pink", shape="circle", label_highlight_color="black")
    else:
